...
class _ConvND(dt.core.TensorBase):
    def __init__(
        ...
        self._conv_strides = normalize_strides(self.rank, strides)

        if self.input.ndim != self.rank + 2:
            raise ValueError(
                f'Input shape must have {self.rank + 2} dimensions, '
                f'got {self.input.ndim}'
            )
        if self.kernel.ndim != self.rank + 2:
            raise ValueError(
                f'Kernel shape must have {self.rank + 2} dimensions, '
                f'got {self.kernel.ndim}'
            )
        if len(self._conv_strides) != self.rank:
            raise ValueError(
                f'Strides must have {self.rank} dimensions, '
                f'got {len(self._conv_strides)}'
            )
    ...
    def compute_gradients(self, grad):
        ...
        *k_shape, c1_, c2 = self.kernel._shape
        ...
        grad_up_shape = [
            batch,
            *(x - k + 1 for x, k in zip(x_shape, k_shape)),
            c2,
        ]
        grad_up = _up_sample_zeros(
            grad,
            [1, *self._conv_strides, 1],
            grad_up_shape,
        )
    ...
    def get_config(self):
        return {'strides': self._conv_strides}
...
class _DepthwiseConv2D(dt.core.TensorBase):
    def __init__(
        ...
        self._conv_strides = normalize_strides(2, strides)
        ...
        if self.kernel.ndim != 4:
            raise ValueError(
                f'Kernel must have 4 dims, got {self.kernel.ndim}'
            )
        ...
        self._shape = conv_output_shape_depthwise(
            self.input._shape,
            self.kernel._shape,
            self._conv_strides,
        )
...
class _UpSampleZeros(dt.core.TensorBase):
    def __init__(
        ...
        for exp, shp in zip(expected_shape, shape):
            if exp < shp:
                raise ValueError(
                    f'Cannot upsample to smaller shape: expected at least '
...
def normalize_strides(
    ...
    if len(strides) != rank:
        raise ValueError(
            f'Strides length {len(strides)} does not match rank {rank}'
        )
    if min(strides) <= 0:
...
def conv_output_shape(
    ...
    *k, C1_, C2 = kernel_shape
    ...
    if C1 != C1_:
        raise ValueError(
            f'Input channels ({C1}) and kernel channels ({C1_}) must match'
        )
    ...
    for inp, ker, stride in zip(x, k, strides):
        ...
        if out <= 0:
            raise ValueError(
                f'Invalid output size computed: '
                f'{(inp - ker)} // {stride} + 1 = {out}'
            )
        result.append(out)
    return tuple([B, *result, C2])


def conv_output_shape_depthwise(
    input_shape: tuple[int, ...],
    kernel_shape: tuple[int, ...],
    strides: tuple[int, ...],
) -> tuple[int, ...]:
    ...
    *k, C_k, M = kernel_shape
    if C != C_k:
        raise ValueError(
            f'Input channels ({C}) and kernel channels ({C_k}) must match'
        )
    result = []
    for inp, ker, stride in zip(x, k, strides):
        out = (inp - ker) // stride + 1
        if out <= 0:
            raise ValueError(
                f'Invalid output size: {(inp - ker)} // {stride} + 1 = {out}'
            )
        result.append(out)
...
def conv_pad(
    rank: int,
    input: dt.core.TensorBase,
    kernel: dt.core.TensorBase,
    strides: tuple[int, ...],
    padding: str,
):
