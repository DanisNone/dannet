...
from typing import (
    Callable, Iterable, Iterator, Sequence, SupportsIndex,
...
class GraphList(Sequence[T], Generic[T]):
    ...
    def __add__(self, other: Sequence[U]) -> GraphList[T | U]:
...
class GraphDict(Generic[K, V]):
    ...
    @overload
    def pop(self, key: K, default: T) -> T | V: ...

    def pop(self, key: K, default: T | None = None) -> T | V:
        try:
            value = self[key]
            del self[key]
            return value
    ...
    def copy(self) -> GraphDict[K, V]:
        result: GraphDict[K, V] = GraphDict()
...
def _graph_eq(x: Any, y: Any) -> bool:
    if hasattr(x, '__graph_eq__'):
        res = x.__graph_eq__(y)
    elif hasattr(y, '__graph_eq__'):
        res = y.__graph_eq__(x)
    else:
        res = x == y
