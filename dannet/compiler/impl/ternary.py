...
def ternary(
    device, node: dt.math._ElementWiseTernary, input_buffers, output_buffer, op: str
):
...
'''
    ...
    local_size = None
    return lambda: kernel.general(device.queue, global_size, local_size, A, B, C, D)


@register_impl(dt.math._Where)
def where(device, node, input_buffers, output_buffer):
    return ternary(device, node, input_buffers, output_buffer, '(bool)x ? (dtypeD)y : (dtypeD)z')


@register_impl(dt.math._Clip)
def clip(device, node, input_buffers, output_buffer):
    return ternary(device, node, input_buffers, output_buffer, '((x > y ? x : y) < z ? (x > y ? x : y) : z)')
